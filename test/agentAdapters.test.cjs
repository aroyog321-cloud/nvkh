...
test("agent adapters are fixed and create inert worker definitions", () => {
  const fakePath = path.join("C:\\", "agent-bin");
  const options = { platform: "linux", env: { PATH: fakePath } };
  const originalStat = require("node:fs").statSync;
  require("node:fs").statSync = candidate => ({ isFile: () => candidate === path.join(fakePath, "codex") });
  ...
  assert.deepEqual(worker, {
    id: worker.id,
    name: "Codex agent",
    command: path.join(fakePath, "codex"),
    args: [],
    cwd: ".",
    autoStart: false
  });
...
test("Windows npm command shims launch through cmd and missing CLIs fail clearly", () => {
  ...
  const fakePath = path.join("C:\\", "npm");
  fs.statSync = candidate => ({ isFile: () => candidate === path.join(fakePath, "gemini.cmd") });
  const options = { platform: "win32", env: { Path: fakePath }, comSpec: "C:\\Windows\\System32\\cmd.exe" };
  assert.equal(resolveAdapterCommand("gemini", options), path.join(fakePath, "gemini.cmd"));
