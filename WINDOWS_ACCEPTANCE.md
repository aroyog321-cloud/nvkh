...
- [ ] Force one renderer crash and confirm Groundstation reloads while every worker PID remains unchanged.
- [ ] Produce terminal output while the renderer is down and confirm bounded replay restores it after reconnect.
- [ ] Crash the renderer four times within 60 seconds; confirm automatic reload pauses and the native Retry/Close decision appears.
- [ ] Choose Retry after crash-loop pause, keep the renderer healthy for 30 seconds, and confirm a later crash receives a fresh retry budget.
- [ ] Inspect recovery diagnostics and confirm they contain no workspace path, command, arguments, terminal output, exception text, or environment values.
...
- [ ] Temporarily deny reads of the owned lock during close; confirm close remains retryable and a second attempt releases it.
