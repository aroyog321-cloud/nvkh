...
function LiveGroundstationView({ sessions, workspace, activity, unseenActivity, selectedId, onSelect, onFocus, onAction, onNavigate, onDismissActivity }) {
  ...
  return <div className="groundstation-view groundstation-reimagined">
    <section className={`live-project-stage tone-${health.tone}`}>
      <div className="gs-command-header">
      ...
      <div className="gs-operations-strip">
...
function NeedsView({ attention, onAction, onFocus }) {
  ...
  return <div className="needs-view needs-decision-room">
    ...
    <section className="attention-lifecycle-bar"><div><span className="section-kicker">ENGINE LIFECYCLE</span><strong>New → Seen → Acting → Verifying → Recovered</strong></div><div><span>{recovered.length} recovered</span><small>Notification policy is managed in Settings</small></div></section>
...
function NotificationSettings() {
  ...
  return <section className="settings-panel settings-panel-wide notification-settings"><div className="settings-panel__head"><Icon name="attention"/><div><h3>Attention & notifications</h3><p>Control when Mission Control interrupts you. Needs You remains focused only on decisions.</p></div></div><div className="attention-policy"><div className="severity-choice"><span>Notify from</span><div>{[["info","All"],["warning","Warning"],["critical","Critical"]].map(([value,label]) => <button key={value} className={policy.minimumSeverity === value ? "is-current" : ""} onClick={() => void save({ ...policy, minimumSeverity: value })}>{label}</button>)}</div></div><label><input type="checkbox" checked={policy.desktopNotifications} onChange={event => void save({ ...policy, desktopNotifications: event.target.checked })}/><span>Desktop notifications</span></label><label><input type="checkbox" checked={policy.quietHours.enabled} onChange={event => void save({ ...policy, quietHours: { ...policy.quietHours, enabled: event.target.checked } })}/><span>Quiet hours</span></label><label className="quiet-hours"><input type="time" value={policy.quietHours.start} onChange={event => setPolicy(current => ({ ...current, quietHours: { ...current.quietHours, start: event.target.value } }))} onBlur={() => void save(policy)}/><span>to</span><input type="time" value={policy.quietHours.end} onChange={event => setPolicy(current => ({ ...current, quietHours: { ...current.quietHours, end: event.target.value } }))} onBlur={() => void save(policy)}/></label></div></section>;
}
...
export default function App() {
  ...
  const renderView = () => {
    ...
    return <div className="settings-composite"><SettingsView state={state} workspace={workspace} recovery={recovery} preferences={preferences} onPreference={updatePreference} onReset={resetPreferences}/><NotificationSettings/><ResourceLinks/></div>;
  };
