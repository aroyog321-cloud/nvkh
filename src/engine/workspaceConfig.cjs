...
class WorkspaceStore {
  ...
  automationDefinitions() {
    return Array.isArray(this.raw.automations) ? this.raw.automations.map(item => JSON.parse(JSON.stringify(item))) : [];
  }

  automationApprovalDefinitions() {
    return Array.isArray(this.raw.automationApprovals) ? this.raw.automationApprovals.map(item => JSON.parse(JSON.stringify(item))) : [];
  }

  automationAuditDefinitions() {
    return Array.isArray(this.raw.automationAudit) ? this.raw.automationAudit.map(item => JSON.parse(JSON.stringify(item))) : [];
  }

  setAutomationState({ automations, approvals, audit }) {
    const nextRaw = this._nextRaw();
    nextRaw.automations = JSON.parse(JSON.stringify(automations || [])).slice(0, 24);
    nextRaw.automationApprovals = JSON.parse(JSON.stringify(approvals || [])).slice(-100);
    nextRaw.automationAudit = JSON.parse(JSON.stringify(audit || [])).slice(-200);
